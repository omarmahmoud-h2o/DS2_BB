"""Model backend: routes LLM calls to vLLM API, vLLM direct, or Anthropic."""
...
class AnthropicBackend:
    ...
    def generate(self, prompt, max_tokens, temperature):
        kwargs = {
            ...
            # Thinking tokens come out of max_tokens, and the caller's budget is
            # sized for the JSON record alone — add headroom so it isn't truncated.
            "max_tokens": min(16000, max_tokens + self.THINKING_HEADROOM),
            "messages": [{"role": "user", "content": prompt}],
        }
        # Claude 5 / 4.6+ models reject `temperature` (400: deprecated); older ones accept it.
        ...
        try:
            ...
            resp = self.client.messages.create(**kwargs)
        # Thinking is on by default on Claude 5 models, so content[0] may be a
        # ThinkingBlock — join the text blocks rather than indexing blindly.
...
def setup_model_backend():
    ...
    if bc["type"] == "vllm_direct":
        ...
        _backend = AnthropicBackend(bc["model_name"], bc["api_key"])
    else:
